...
exports.createFZ = (personID, email, adressID = -1) => {
    createFZWithData(personID, adressID).then(file => {
        mysql_1.query(`SELECT vorname, nachname, geschlecht FROM personen WHERE personID = ${personID}`).then(rows => {
            const p = rows[0];
            mail_1.default('fz@ec-nordbund.de', { to: email, bcc: 'fz@ec-nordbund.de;datenschutz@ec-nordbund.de' }, 'Erweitertes Führungszeugnis', `<p>Hey <b>${p.vorname} ${p.nachname}</b>,</p>
        <p>du bist als Mitarbeiter${p.geschlecht === 'w' ? 'in' : ''} im EC-Nordbund überregional oder in deiner Gemeinschaft/Gemeinde vor Ort tätig. Das freut uns ungemein. <b>Danke für deinen Einsatz.</b><br>
        ...
        <p><b>ThomaS:-)</b></p>`, true, [{ content: file, filename: 'fzAntrag.pdf' }]);
        });
    });
};
function createFZWithData(personID, adressID = -1) {
    return new Promise((res, rej) => {
        if (adressID === -1) {
            mysql_1.query(`SELECT adressID FROM adressen WHERE personID = ${personID} AND isOld = 0 ORDER BY lastUsed LIMIT 1`).then((row) => {
                if (row.length === 0) {
            ...
            mysql_1.query(`SELECT strasse, plz, ort FROM adressen WHERE adressID = ${adressID}`).then((adressen) => {
                mysql_1.query(`SELECT vorname, nachname, gebDat, geschlecht FROM personen WHERE personID = ${personID}`).then((personen) => {
                    const p = personen[0];
                    const a = adressen[0];
...
exports.createFZWithData = createFZWithData;
function createFZDocument(vorname, nachname, gebDat, strasse, plz, ort, geschlecht) {
    const settings = `
...
\\end{document}
  `;
    return new Promise((res, rej) => {
        let pdf = latex_1.latex(latexCode);
        console.log(__dirname);
        pdf.pipe(fs_1.createWriteStream(path_1.join(__dirname, '../../tmp/output.pdf')));
        pdf.on('error', rej);
        pdf.on('finish', () => {
            res(fs_1.readFileSync(path_1.join(__dirname, '../../tmp/output.pdf')).toString('base64'));
        });
    });
}
